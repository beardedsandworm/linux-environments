...
setup_wormlogic_vpn() {
  ...
  if [[ -z "${WORMLOGIC_VPN_IP:-}" ]]; then
    echo
    read -r -p "server03 VPN IP [$default_vpn_ip]: " input_vpn_ip
...
show_summary() {
  ...
  if [[ -n "${WORMLOGIC_VPN_PUBLIC_KEY:-}" ]]; then
    ...
    echo
    echo "  # ${WORMLOGIC_VPN_MACHINE_ID:-server03}"
    echo "  [Peer]"
    echo "  PublicKey = $WORMLOGIC_VPN_PUBLIC_KEY"
    echo "  AllowedIPs = ${WORMLOGIC_VPN_IP:-REPLACE_WITH_SERVER03_VPN_IP}"
    echo
